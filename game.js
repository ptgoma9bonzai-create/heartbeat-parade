(() => {
'use strict';
const c=document.querySelector('#game'),g=c.getContext('2d'),W=480,H=720,PHONE_RENDER=(window.matchMedia?.('(pointer:coarse)').matches||window.innerWidth<760),RENDER_SCALE=Math.min(PHONE_RENDER?1.3:2,Math.max(1,window.devicePixelRatio||1));c.width=W*RENDER_SCALE;c.height=H*RENDER_SCALE;g.setTransform(RENDER_SCALE,0,0,RENDER_SCALE,0,0);
...
const MAX_PARTICLES=PHONE_RENDER?140:220,MAX_NOTES=PHONE_RENDER?48:64,MAX_CALLOUTS=18,MAX_ITEMS=18,MAX_CONTINUES=2;
...
const DIFFICULTIES={
 easy:{label:'EASY',enemy:.9,bullets:1.05,speed:.88,hp:1.12,bossHp:1,fire:.9,drops:{weapon:.9,bomb:.012,heal:.02,star:.025,all:.008}},
 normal:{label:'NORMAL',enemy:1,bullets:1.3,speed:1,hp:1.3,bossHp:1.3,fire:1,drops:{weapon:.72,bomb:.006,heal:.012,star:.015,all:.01}},
 hard:{label:'HARD',enemy:1.12,bullets:1.34,speed:1.1,hp:1.45,bossHp:1.55,fire:1.12,drops:{weapon:.72,bomb:.005,heal:.012,star:.01,all:.018}},
 hell:{label:'HELL',enemy:1.6,bullets:2.15,speed:1.33,hp:2,bossHp:2.05,fire:1.55,drops:{weapon:.62,bomb:.0035,heal:.008,star:.007,all:.016}}
};
const STAGE_TUNING={
 easy:{enemy:[.9,.91,.92,.93,.94,.95],bullets:[1.05,1.05,1.06,1.06,1.07,1.08],speed:[.88,.88,.89,.89,.9,.9],fire:[.9,.9,.91,.91,.92,.92],cap:[150,160,170,180,190,200]},
 normal:{enemy:[1,1,1,1,1,1],bullets:[1.24,1.26,1.28,1.28,1.3,1.3],speed:[1,1,1,1,1,1],fire:[1,1,1,1,1,1],cap:[185,200,215,225,240,250]},
 hard:{enemy:[1.06,1.07,1.07,1.06,1.05,1.05],bullets:[1.28,1.3,1.31,1.3,1.31,1.32],speed:[1.06,1.06,1.07,1.06,1.06,1.06],fire:[1.05,1.06,1.07,1.06,1.06,1.07],cap:[200,215,230,240,250,265]},
 hell:{enemy:[1.24,1.22,1.2,1.17,1.13,1.08],bullets:[1.48,1.5,1.52,1.5,1.52,1.55],speed:[1.18,1.18,1.17,1.17,1.16,1.15],fire:[1.2,1.21,1.22,1.21,1.2,1.18],cap:[230,245,260,270,285,300]}
};
...
const ENEMY_LIMITS={easy:[6,7,8,8,9,9],normal:[7,8,9,9,10,9],hard:[8,9,10,10,11,10],hell:[9,10,11,11,12,11]};
...
const diff=()=>DIFFICULTIES[selectedDifficulty];let tuneCacheKey='',tuneCache=null;const tune=()=>{const keyName=selectedDifficulty+'-'+stage;if(keyName===tuneCacheKey&&tuneCache)return tuneCache;const t=STAGE_TUNING[selectedDifficulty],i=clamp(stage,0,5);tuneCacheKey=keyName;return tuneCache={enemy:t.enemy[i],bullets:t.bullets[i],speed:t.speed[i],fire:t.fire[i],cap:t.cap[i]}},bulletLimit=()=>Math.min(tune().cap,PHONE_RENDER?300:360),lowFx=()=>PHONE_RENDER||selectedDifficulty==='hell'||enemyShots.length>220,ship=()=>SHIPS[selectedShip];
...
function showContinue(loadout){
 if(continuesUsed>=MAX_CONTINUES){gameOver();return}mode='continue';continueLoadout=loadout;pointer=false;activePointer=null;activeTouches.clear();key.clear();clearInterval(musicTimer);title.innerHTML='CONTINUE?';msg.innerHTML='STAGE '+(stage+1)+'「'+STAGES[stage].name+'」を最初から再開<br>FULL POWER BONUS：BOMB 5・ALL WEAPON Lv3<br>残りコンティニュー '+(MAX_CONTINUES-continuesUsed)+' 回';btn.textContent='生還: Try Again';endBtn.hidden=false;gameSelect.classList.add('locked');over.classList.remove('hidden');
}
function continueGame(){
 if(mode!=='continue'||!stageCheckpoint)return;continuesUsed++;const cp=stageCheckpoint,loadout=continueLoadout||cp;stage=cp.stage;score=cp.score;stageTime=0;combo=1;comboClock=0;spawnClock=stage===5?.5:.7;trapClock=stage===5?4:5;skillDodges=cp.skillDodges||0;bossMade=false;midBossMade=false;bombDrops=0;beat=0;bossShield=null;bombFx=null;deathBorder=null;beamFx=[];shots=[];enemies=[];enemyShots=[];particles=[];notes=[];items=[];traps=[];mineLines=[];callouts=[];lastHudState='';player={x:W/2,y:H-90,r:3.5,hp:5,bombs:5,cd:0,laserCd:0,missileCd:0,inv:2,power:0,starBombReady:false,recovery:0,trail:[],drones:[],droneCd:0,funnelGauge:Math.max(cp.funnelGauge||1,loadout.funnelGauge||1),levels:{shot:3,laser:3,missile:3}};continueLoadout=null;banner=2.5;callouts.push({x:W/2,y:118,text:'FULL POWER CONTINUE '+continuesUsed+'/'+MAX_CONTINUES,life:2.2,color:'#8ffaff'});hud();launchPlay();
}
...
function reset(){
 ...
 player={x:W/2,y:H-90,r:3.5,hp:5,bombs:3,cd:0,laserCd:0,missileCd:0,inv:1,power:0,starBombReady:false,recovery:0,trail:[],drones:[],droneCd:0,funnelGauge:1,levels:{shot:1,laser:0,missile:0}};
...
function burst(x,y,color,n=12){if(lowFx())n=Math.max(3,Math.ceil(n*.56));const limit=selectedDifficulty==='hell'?Math.min(MAX_PARTICLES,150):MAX_PARTICLES;n=Math.min(n,limit);const overflow=particles.length+n-limit;if(overflow>0)particles.splice(0,overflow);while(n--){const a=R(0,7),v=R(40,220);particles.push({x,y,vx:Math.cos(a)*v,vy:Math.sin(a)*v,l:R(.3,.9),s:R(2,6),color})}}
function spawnItem(kind,x,y,vy=64){if(items.length>=MAX_ITEMS)items.shift();items.push({x:clamp(x,20,W-20),y,r:kind==='all'?15:14,kind,phase:R(0,7),vy})}
function dropItem(e){
 const drops=diff().drops,offset=()=>R(-18,18),recovery=1+(player.recovery||0)*.22;
 if(e.type!=='boss'){const support=Math.random(),star=drops.star,heal=star+drops.heal*(player.hp<5?1.35:1),bomb=heal+drops.bomb*(player.bombs<5?1.3:1);if(support<star)spawnItem('star',e.x+offset(),e.y-12,62);else if(support<heal)spawnItem('heal',e.x+offset(),e.y,64);else if(support<bomb)spawnItem('bomb',e.x+offset(),e.y+8,60)}
 if(Math.random()<Math.min(.16,drops.all*recovery*(e.type==='tough'?1.5:1)))spawnItem('all',e.x+offset(),e.y-4,68);
 else if(e.type==='boss'||Math.random()<Math.min(.82,(e.type==='tough'?.6:.16)*drops.weapon*recovery)){const types=['shot','laser','missile'],kind=types[Math.floor(R(0,types.length))];spawnItem(kind,e.x,e.y,75)}
}
function dropMidBossRewards(e){
 spawnItem('heal',e.x-18,e.y,62);
 const quota=stage<3?1:2;while(bombDrops<quota){spawnItem('bomb',e.x+18+(bombDrops*18),e.y-8*bombDrops,58);bombDrops++}
}
function bossSupplyMarks(){return {easy:[.9,.72,.54,.36,.18],normal:[.9,.66,.42,.2],hard:[.9,.68,.46,.24],hell:[.9,.67,.44,.22]}[selectedDifficulty].slice()}
function dropBossSupply(e){
 const types=['shot','laser','missile'],missing=Math.max(...types.map(k=>3-player.levels[k])),pool=types.filter(k=>3-player.levels[k]===missing),kind=(player.recovery||0)>=2?'all':pool[Math.floor(R(0,pool.length))];spawnItem(kind,e.x+R(-34,34),e.y+Math.min(48,e.r||30),48);callouts.push({x:e.x,y:e.y+54,text:kind==='all'?'ALL WEAPON SUPPORT!':'POWER SUPPORT!',life:1.45,color:kind==='all'?'#baff72':WEAPONS[kind].color});tone(1046,.18,'triangle',.025);
}
...
function updateDrones(dt){
 player.droneCd-=dt;const bulletStep=lowFx()?2:1;for(const d of player.drones){d.inv=Math.max(0,d.inv-dt);d.phase+=dt*(1.35+Math.sin(d.phase)*.18);let tx=player.x+Math.cos(d.phase)*58,ty=player.y-18+Math.sin(d.phase)*34,ax=0,ay=0;for(let i=0;i<enemyShots.length;i+=bulletStep){const b=enemyShots[i],dx=d.x-b.x,dy=d.y-b.y,dist=Math.hypot(dx,dy);if(dist<105&&dist>1){const risk=(105-dist)/105;ax+=dx/dist*risk;ay+=dy/dist*risk}}const avoid=Math.hypot(ax,ay);if(avoid){tx+=ax/avoid*42;ty+=ay/avoid*34}d.x=clamp(d.x+(tx-d.x)*6.5*dt,16,W-16);d.y=clamp(d.y+(ty-d.y)*6.5*dt,32,H-30)}
...
const REGULAR_ENEMY_TYPES=new Set(['small','tough','spaceRay','scrap']),isRegularEnemy=e=>e&&REGULAR_ENEMY_TYPES.has(e.type);
function spawnEnemyX(radius){const top=enemies.filter(e=>!e.dead&&isRegularEnemy(e)&&e.y<190);let best=R(radius+20,W-radius-20),bestGap=-1;for(let attempt=0;attempt<8;attempt++){const x=R(radius+20,W-radius-20),gap=top.length?Math.min(...top.map(e=>Math.abs(x-e.x)-e.r)):W;if(gap>bestGap){best=x;bestGap=gap}if(gap>radius+32)break}return best}
function separateEnemies(){const crowd=enemies.filter(e=>!e.dead&&isRegularEnemy(e));for(let i=0;i<crowd.length;i++)for(let j=i+1;j<crowd.length;j++){const a=crowd[i],b=crowd[j],min=a.r+b.r+8,dy=Math.abs(a.y-b.y);if(dy>min*.78)continue;const dx=a.x-b.x,adx=Math.abs(dx);if(adx>=min)continue;const push=(min-adx)*.52,dir=dx>=0?1:-1;a.x=clamp(a.x+dir*push,a.r,W-a.r);b.x=clamp(b.x-dir*push,b.r,W-b.r)}}
function addEnemy(type=Math.random()<TOUGH_CHANCE[selectedDifficulty][stage]?'tough':'small'){
 ...
 if(type==='spaceRay'||type==='scrap'){const scrap=type==='scrap',r=scrap?27:24,hp=(scrap?24:11)*diff().hp;enemies.push({x:spawnEnemyX(r),y:-42,r,hp,max:hp,v:scrap?62:88,phase:R(0,7),shoot:R(.65,1.15),type,hellMove:Math.floor(R(0,3)),worth:scrap?1150:760});return}
 const tough=type==='tough',scale=stage<2?stage:1.4+(stage-2)*.3,hp=tough?10+scale*7:2+scale*2;
 const r=tough?24:15,max=hp*diff().hp;enemies.push({x:spawnEnemyX(r),y:-35,r,hp:max,max,v:(tough?55:85)*(.82+stage*.18),phase:R(0,7),shoot:R(1.1,2)/(1+stage*.15),type,hellMove:Math.floor(R(0,3)),worth:tough?800:180});
}
function addMidBoss(){
 const hp=(90+stage*55)*diff().hp;
 midBossMade=true;for(const e of enemies)if(isRegularEnemy(e))e.dead=true;enemies=enemies.filter(e=>!e.dead);enemyShots=[];
...
function addBoss(){
 const hp=[480,680,820,980,1450,1750][stage]*diff().bossHp,r=[66,72,78,86,112,176][stage];for(const e of enemies)if(isRegularEnemy(e))e.dead=true;enemies=enemies.filter(e=>!e.dead);bossMade=true;beat=0;enemyShots=[];traps=[];mineLines=[];trapClock=999;beamFx=[];deathBorder=null;
...
function pushEnemyShot(b){if(enemyShots.length>=bulletLimit())return false;enemyShots.push(b);return true}
function hazard(e,q,speed,kind='orb',extra={}){
 if(enemyShots.length>=bulletLimit())return;speed*=tune().speed;pushEnemyShot({x:e.x,y:e.y,r:kind==='bomb'||kind==='bubble'?11:kind==='tentacle'?8:kind==='yoyo'?9:6,vx:Math.cos(q)*speed,vy:Math.sin(q)*speed,color:STAGES[stage].bullet,shape:stage,kind,spin:extra.spin||0,trail:[],...extra});
}
...
function enemyFire(e){
 ...
 for(let i=0;i<count;i++){const q=base+(stage>=2&&e.type!=='small'?i:(i-(count-1)/2))*spread;pushEnemyShot({x:e.x,y:e.y,r:e.type==='boss'?6:4.5,vx:Math.cos(q)*speed*tune().speed,vy:Math.sin(q)*speed*tune().speed,color:STAGES[stage].bullet,shape:stage,spin:spin*(i%2?1:-1),trail:[]})}
...
function dropMineReward(t){const types=['shot','laser','missile'],missing=types.reduce((sum,k)=>sum+3-player.levels[k],0),roll=Math.random();let kind;if(player.hp<=2&&roll<.28)kind='heal';else if(player.bombs<=2&&roll<.48)kind='bomb';else if((player.recovery||0)>=2&&roll<.72)kind='all';else if(missing>0){const need=Math.max(...types.map(k=>3-player.levels[k])),pool=types.filter(k=>3-player.levels[k]===need);kind=pool[Math.floor(R(0,pool.length))]}else kind=roll<.55?'bomb':'star';spawnItem(kind,t.x,t.y,54);callouts.push({x:t.x,y:t.y-48,text:'MINE ITEM!',life:1.5,color:kind==='all'?'#baff72':'#bfe9d4'})}
function damageBreakableMine(t,amount,color='#8dcfaf'){
 if(t.dead||!t.breakable)return;t.hp-=amount;burst(t.x,t.y,color,5);if(t.hp<=0){t.dead=true;score+=2500;shake=8;burst(t.x,t.y,'#8dcfaf',40);dropMineReward(t);callouts.push({x:t.x,y:t.y-26,text:'MINE BREAK!',life:1.8,color:'#bfe9d4'});tone(980,.2,'triangle',.025)}
}
...
function damage(){
 if(player.inv>0||player.power>0)return;const rescue={bombs:player.bombs,levels:{...player.levels},funnelGauge:player.funnelGauge};player.hp--;player.inv=2;player.drones=[];player.levels.shot=Math.max(1,player.levels.shot-1);player.levels.laser=Math.max(0,player.levels.laser-1);player.levels.missile=Math.max(0,player.levels.missile-1);player.recovery=Math.min(6,(player.recovery||0)+3);combo=1;comboClock=0;enemyShots=[];shake=15;burst(player.x,player.y,'#fff',30);tone(95,.5,'sawtooth',.05);hud();if(player.hp<=0)showContinue(rescue);
}
function update(dt){
 ...
 player.trail.unshift({x:player.x,y:player.y,l:.3});player.trail=player.trail.slice(0,lowFx()?5:9);for(const q of player.trail)q.l-=dt;
 updateDrones(dt);
 if(player.cd<=0)fireWeapon();
 const midAt=stage===5?7:12,bossAt=stage===5?18:28;
 if(!midBossMade&&stageTime>=midAt)addMidBoss();
 if(!bossMade&&stageTime<bossAt&&(spawnClock-=dt)<=0){const midAlive=enemies.some(e=>e.type==='midboss'&&!e.dead),regularCount=enemies.reduce((n,e)=>n+(!e.dead&&isRegularEnemy(e)?1:0),0);if(!midAlive&&regularCount<ENEMY_LIMITS[selectedDifficulty][stage])addEnemy();spawnClock=SPAWN_INTERVAL[stage]*R(.88,1.12)/tune().enemy}
 ...
 for(const it of items){it.y+=it.vy*dt;it.phase+=dt*4;if(hit(player,it)){it.dead=true;if(it.kind==='heal'){player.hp=Math.min(5,player.hp+1);burst(it.x,it.y,'#ff668e',30);tone(880,.5,'sine',.05)}else if(it.kind==='bomb'){player.bombs=Math.min(5,player.bombs+1);burst(it.x,it.y,'#fff1a6',30);tone(740,.15,'square',.035);tone(1046,.3,'triangle',.035)}else if(it.kind==='star'){player.power=7;player.starBombReady=true;player.inv=Math.max(player.inv,7);burst(it.x,it.y,'#fff36b',55);callouts.push({x:player.x,y:player.y-48,text:'STAR BOMB READY ×1.5',life:1.8,color:'#fff36b'});[784,988,1175,1568].forEach((f,i)=>setTimeout(()=>tone(f,.3,'triangle',.035),i*70))}else if(it.kind==='all'){for(const k of ['shot','laser','missile'])player.levels[k]=Math.min(3,player.levels[k]+1);player.recovery=Math.max(0,(player.recovery||0)-3);burst(it.x,it.y,'#baff72',40);callouts.push({x:player.x,y:player.y-48,text:'ALL WEAPON +1',life:1.5,color:'#baff72'});tone(1046,.2,'triangle',.045);tone(1568,.35,'triangle',.035)}else{player.levels[it.kind]=Math.min(3,player.levels[it.kind]+1);player.recovery=Math.max(0,(player.recovery||0)-1);burst(it.x,it.y,WEAPONS[it.kind].color,24);tone(1046,.35,'triangle',.045)}score+=500;hud()}}
...
function drawPlayer(){
 ...
 g.save();g.translate(player.x,player.y);g.globalAlpha=player.inv>0&&Math.floor(player.inv*12)%2?.35:1;g.shadowBlur=lowFx()?6:18;g.shadowColor='#fff';
 g.scale(.76,.76);
...
function drawEnemy(e){
 g.save();g.translate(e.x,e.y);g.shadowBlur=lowFx()&&e.type!=='boss'?3:15;g.shadowColor=STAGES[stage].accent;
 if(e.type==='boss'){
...
function draw(){
 ...
 g.globalAlpha=.55;for(let i=0;i<stars.length;i+=lowFx()?2:1){const st=stars[i];g.fillStyle='#fff';g.beginPath();g.arc(st.x,st.y,st.s,0,7);g.fill()}
 ...
 if(!lowFx()){g.globalAlpha=.1;g.strokeStyle='#fff';g.lineWidth=2;for(let y=(time*70)%120-120;y<H;y+=120){g.beginPath();g.moveTo(0,y);g.bezierCurveTo(130,y+55,350,y-35,W,y+20);g.stroke()}}
 ...
 g.globalAlpha=1;for(const b of shots){
  ...
  if(b.type==='laser'){const w=b.width||5;if(b.ship==='ribbon'){g.fillStyle='#ff88cf';g.beginPath();g.moveTo(0,-32);g.quadraticCurveTo(w*1.4,-12,0,4);g.quadraticCurveTo(-w*1.4,20,0,34);g.quadraticCurveTo(w*1.4,16,0,0);g.quadraticCurveTo(-w*1.4,-16,0,-32);g.fill();g.fillStyle='#fff';g.fillRect(-2,-34,4,68)}else if(b.ship==='hammer'){g.fillStyle='#92ff5f';g.fillRect(-w,-25,w*2,50);g.fillStyle='#fff7a6';g.fillRect(-w*.48,-31,w*.96,62);g.strokeStyle='#456b36';g.lineWidth=3;g.strokeRect(-w,-25,w*2,50)}else{g.fillStyle='#bffcff';g.fillRect(-w,-24,w*2,48);g.fillStyle='#fff';g.fillRect(-w*.42,-29,w*.84,58)}}
   ...
   g.rotate(Math.atan2(b.vy,b.vx)+Math.PI/2);const flame=6+Math.sin(time*38+b.x)*3;g.lineCap='round';g.globalAlpha=.55;g.strokeStyle=b.ship==='hammer'?'#ff7a32':b.ship==='ribbon'?'#ff9ada':'#79efff';g.lineWidth=b.ship==='hammer'?7:5;g.beginPath();g.moveTo(0,10);g.lineTo(0,28+flame);g.stroke();g.globalAlpha=1;g.strokeStyle='#34264d';g.lineWidth=2.5;
   if(b.ship==='ribbon'){g.fillStyle='#ff70bd';g.beginPath();g.ellipse(0,-1,9,17,0,0,7);g.fill();g.stroke();g.fillStyle='#fff';g.beginPath();g.moveTo(0,-20);g.lineTo(7,-7);g.lineTo(-7,-7);g.closePath();g.fill();g.stroke();g.strokeStyle='#ffb3df';g.lineWidth=3;g.beginPath();g.moveTo(-5,10);g.quadraticCurveTo(-17,20,-8,30);g.moveTo(5,10);g.quadraticCurveTo(17,20,8,30);g.stroke();heart(0,1,8,'#fff1f8')}
   else if(b.ship==='hammer'){g.fillStyle='#8be35f';g.fillRect(-10,-10,20,23);g.strokeRect(-10,-10,20,23);g.fillStyle='#fff4a6';g.beginPath();g.moveTo(0,-24);g.lineTo(12,-8);g.lineTo(-12,-8);g.closePath();g.fill();g.stroke();g.fillStyle='#7051a0';g.fillRect(-16,0,32,9);g.strokeRect(-16,0,32,9);g.fillStyle='#ff6b32';g.fillRect(-7,13,14,9)}
   else{g.fillStyle='#e9fbff';g.beginPath();g.moveTo(0,-20);g.lineTo(9,-4);g.lineTo(7,12);g.lineTo(0,8);g.lineTo(-7,12);g.lineTo(-9,-4);g.closePath();g.fill();g.stroke();g.fillStyle='#ff648e';g.beginPath();g.moveTo(-8,2);g.lineTo(-17,12);g.lineTo(-6,9);g.moveTo(8,2);g.lineTo(17,12);g.lineTo(6,9);g.fill();g.stroke();heart(0,-4,8,'#6ff8ff')}
   g.fillStyle='#2f2447';g.font='900 8px sans-serif';g.textAlign='center';g.textBaseline='middle';g.fillText('M',0,b.ship==='hammer'?3:4)
  }
 ...
 for(const it of items){const color=it.kind==='heal'?'#ff668e':it.kind==='bomb'?'#fff1a6':it.kind==='star'?'#fff36b':it.kind==='all'?'#baff72':WEAPONS[it.kind].color;g.save();g.translate(it.x,it.y);g.rotate(it.kind==='star'?it.phase:Math.sin(it.phase)*.18);g.shadowBlur=lowFx()?4:18;g.shadowColor=color;g.fillStyle='#fff8e8';g.strokeStyle=color;g.lineWidth=4;g.beginPath();if(it.kind==='star'){for(let i=0;i<10;i++){const a=-Math.PI/2+i*Math.PI/5,r=i%2?6:14;g.lineTo(Math.cos(a)*r,Math.sin(a)*r)}g.closePath()}else g.arc(0,0,it.kind==='all'?15:14,0,7);g.fill();g.stroke();if(it.kind==='heal')heart(0,0,11,'#ff668e');else if(it.kind!=='star'){if(it.kind==='all'){g.strokeStyle='#6ff8ff';g.lineWidth=2;g.beginPath();g.arc(0,0,10,it.phase,it.phase+4.6);g.stroke()}g.fillStyle='#51416d';g.font='900 13px sans-serif';g.textAlign='center';g.textBaseline='middle';g.fillText(it.kind==='bomb'?'B':it.kind==='shot'?'S':it.kind==='laser'?'L':it.kind==='all'?'A':'M',0,1)}g.restore()}
 for(const e of enemies)if(!e.dead)drawEnemy(e);drawBossShield();
 for(const b of enemyShots){
  for(let i=0;i<b.trail.length;i++){g.globalAlpha=(1-i/b.trail.length)*.2;g.fillStyle=b.color;g.beginPath();g.arc(b.trail[i].x,b.trail[i].y,b.r*(1-i/b.trail.length),0,7);g.fill()}g.globalAlpha=1;g.save();g.translate(b.x,b.y);g.rotate(b.angle||0);g.shadowBlur=lowFx()?0:enemyShots.length>180?3:9;g.shadowColor=b.color;g.fillStyle=b.color;g.strokeStyle='#fff';g.lineWidth=b.kind==='mineLine'?1:1.5;g.beginPath();
